        ...
        if order.payment_method == "usdt" and order.payment_amount:
            ...
            if not paid:
                hints = {
                    ...
                    "oauth_forbidden": (
                        "У токена нет права operation-history. "
                        "В админке → Настройки → заново откройте авторизацию ЮMoney "
                        "и разрешите просмотр истории операций."
                    ),
                    "not_found": (
                        "Платёж пока не виден в истории ЮMoney "
                        "(учитывается комиссия, например 50→48,50). "
                        "Подождите 20–40 сек — проверка идёт сама."
                    ),
