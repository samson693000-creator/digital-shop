...
После установки откройте админку → **Настройки** → вставьте `BOT_TOKEN` → в меню слева **↻ Рестарт**.
...
- Автовыдача ключа/текста/ссылки/файла после оплаты
- Оплата: USDT TRC-20, ЮMoney, баланс
- Реферальная программа и личный кабинет
- Помощь / обратная связь с ответом админа в боте

**Админка**
- Товары, категории, пакетная загрузка ключей
- Фото товара + описание из .txt + выдача файлами (фото/pdf/zip…)
- Бесконечные товары (∞), правка цены у существующих
